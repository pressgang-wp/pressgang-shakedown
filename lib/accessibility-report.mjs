...
export function highlight(evidence, node) {
  ...
  const svg = view => `<svg xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Element outlined on the captured page" viewBox="${view}"><image href="${src}" width="${w}" height="${h}"/><rect x="${x}" y="${y}" width="${right - x}" height="${bottom - y}" fill="none" stroke="white" stroke-width="6" vector-effect="non-scaling-stroke"/><rect x="${x}" y="${y}" width="${right - x}" height="${bottom - y}" fill="none" stroke="#b00020" stroke-width="3" vector-effect="non-scaling-stroke"/></svg>`;
