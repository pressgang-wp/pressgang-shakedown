...
export function reviewGroups(result) {
  ...
  if (differences.some(d => !levels.core.differenceKeys.includes(d.key)) || accessibility.some(f => !f.blocking) || result.visual?.changed || result.visual?.percent > 0 || result.visual?.unavailable) groups.push('other');
...
export function observationVerdict(result) {
  ...
  if (result.visual?.changed || result.visual?.percent > 0 || result.visual?.unavailable || result.differences.some(d => !d.suppressed) ||
