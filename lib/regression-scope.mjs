...
export function selectViewports(configured, defaults, flag) {
  ...
  const names = flag === undefined
    ? defaults ?? configured?.map(v => v.name) ?? ['desktop', 'mobile']
...
export function parseRegressionFlags(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    const match = args[i].match(/^--(against|candidate|level|viewports)(?:=(.*))?$/);
    if (!match) throw new Error('Unsupported regression argument: ' + args[i] + '. Use --against, --candidate, --level or --viewports.');
...
export function reviewGroups(result) {
  ...
  if (differences.some(d => !levels.core.differenceKeys.includes(d.key)) || accessibility.some(f => !f.blocking)) groups.push('other');
...
export const reviewLabels = {
  ...
  core: 'Core changes for review',
  other: 'Other changes and advisories',
...
export function observationVerdict(result) {
  ...
  if (result.differences.some(d => !d.suppressed) ||
