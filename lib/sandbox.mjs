...
export function seedThemeMuster(sandbox, { seed = 42 } = {}) {
  const themeDir = join(sandbox.root, 'wp-content', 'themes', sandbox.theme ?? '');
  // Seeders live in a top-level `muster/` (autoload-dev); `src/Muster/` is the
  // pre-convention location, still honoured so older themes keep working.
  const hasSeeders =
    sandbox.theme && (existsSync(join(themeDir, 'muster')) || existsSync(join(themeDir, 'src', 'Muster')));
