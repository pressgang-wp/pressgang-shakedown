...
export async function getEvidence(request, url, { timeout = 20000, maxRedirects = 5 } = {}) {
  ...
  for (let i = 0; i <= maxRedirects; i++) {
    ...
    // Playwright's reused-TLS-socket certificate inspection can throw outside
    // the request promise when certificate metadata is absent. Avoid pooling
    // these anonymous requests; do not weaken certificate verification.
    const res = await request.get(current, { maxRedirects: 0, timeout, headers: { cookie: '', authorization: '', connection: 'close' } });
  ...
  await context.route('**/*', async route => {
    ...
    try {
      ...
      for (let i = 0; i <= 5; i++) {
        ...
        const headers = { ...req.headers(), cookie: '', authorization: '', connection: 'close' };
...
export async function semanticEvidence(page, ignoreSelectors = [], level = 'full') {
  return page.evaluate(({ selectors, level }) => {
    ...
    return {
      ...
      structureScope: { selector: 'main', count: all('main').length },
