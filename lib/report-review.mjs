...
export function renderVisual(visual) {
  ...
  if (visual.engine) return `<section><h3>${visual.changed ? 'Appearance changed' : 'No visual change detected'}</h3><p>${esc(dimensions)}</p><p>Compared by Playwright. Expected is this run’s reference capture; actual is the candidate. A mismatch needs review, not automatic rejection.</p><p><a href="${esc(visual.report)}">Open Playwright visual comparison</a>${visual.screenshot ? ` · <a href="${esc(visual.screenshot)}">Open diff image</a>` : ''}</p></section>`;
  return `<details><summary>Screenshot comparison details</summary><p>${esc(dimensions)}</p>${visual.unavailable ? `<p>Unavailable: ${esc(visual.unavailable)}</p>` : `<a href="${esc(visual.screenshot)}">Open legacy diff image</a>`}</details>`;
}
