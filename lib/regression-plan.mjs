...
export function regressionOptions(raw, discovery, flags = {}) {
  ...
  const lists = {};
  const routes = flags.routes === undefined ? undefined : flags.routes.split(',').map(value => {
    if (!value.startsWith('/') || value.includes('#')) throw new Error('--routes requires exact local paths without fragments');
    return routePath(value, discovery);
  });
  ...
  return { routes, coverage, level, referenceName, candidateName, reference, candidate, discovery: origin(discovery), viewports: selectViewports(configuredViewports, raw.defaultViewports, flags.viewports), ...lists, navigationLimit, timeout };
}

export function focusRoutes(plan, inventory) {
  if (!plan.options.routes) return;
  const wanted = new Set(plan.options.routes);
  for (const path of wanted) {
    if (plan.routes.some(r => r.path === path)) continue;
    const item = inventory?.eligible?.find(r => r.path === path);
    if (!item) throw new Error(`Requested route was not discovered or is excluded: ${path}`);
    plan.routes.push({ ...item, expect: 200, referenceUrl: plan.options.reference + path, candidateUrl: plan.options.candidate + path, source: 'public content inventory (focused selection)' });
  }
  plan.focusExcluded = plan.routes.filter(r => !wanted.has(r.path)).map(r => ({ path: r.path, reason: '--routes selection' }));
  plan.routes = plan.routes.filter(r => wanted.has(r.path));
}
