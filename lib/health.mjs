...
export async function brokenImages(page, { includeLazy = false } = {}) {
  return page.evaluate(includeLazy => Array.from(document.images)
    .filter(img => (img.loading !== 'lazy' || (includeLazy && img.getBoundingClientRect().width > 0)) && img.complete && img.naturalWidth === 0 && !!img.src)
    .map(img => `broken image: ${img.currentSrc || img.src}`), includeLazy);
}
export async function accessibilityFindings(page, ignore = {}) {
  ...
  return violations.map(v => ({
    blocking: ['serious', 'critical'].includes(v.impact),
    message: `[${v.impact}] ${v.id} × ${v.nodes.length} — ${v.helpUrl}`,
    ...
    nodes: v.nodes.map(n => ({
      ...
      checks: Object.fromEntries(['any', 'all', 'none'].map(key => [key, n[key].map(c => ({
        id: c.id, impact: c.impact, message: c.message, data: c.data,
