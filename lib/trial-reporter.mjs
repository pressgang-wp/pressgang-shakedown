...
export default class TrialReporter {
  ...
  onEnd() {
    ...
    const entries = [...this.results.values()].map(({ test, ...route }) => ({
      ...route, ...verdict(test), accessibility: collectAccessibility(test, dir),
    }));
    ...
    try {
      ...
      ignore = m.ignore ?? {};
      discovery = { warnings: m.discoveryWarnings ?? [], excluded: m.discoveryExcluded ?? [] };
    } catch {
...
${run.results.map(r => (r.accessibility ?? []).filter(a => a.findings?.length || a.captureError).map(a => `<h2>${esc(r.kind)} · ${esc(r.url)} · attempt ${a.attempt}</h2>${a.captureError && !a.findings?.length ? `<p>${esc(a.captureError)}</p>` : renderAccessibilityEvidence(a)}`).join('')).join('')}
...
${run.discovery?.excluded?.length || run.discovery?.warnings?.length ? `<h2>Discovery exclusions and warnings</h2><pre>${esc(JSON.stringify(run.discovery, null, 2))}</pre>` : ''}
...
// Keep each attempt's evidence; a passing retry must not erase a failed capture.
function collectAccessibility(test, dir) {
  return test.results.flatMap((result, index) => {
    const attachment = (result.attachments ?? []).find(a => a.name === 'accessibility-evidence');
    if (!attachment) return [];
    try {
      const evidence = JSON.parse(attachment.body ? attachment.body.toString() : readFileSync(attachment.path, 'utf8'));
      const image = result.attachments.find(a => a.name === 'accessibility-screenshot');
      delete evidence.screenshot;
      if (image) {
        const name = createHash('sha256').update(test.id).digest('hex').slice(0, 20) + '-' + index + '.png';
        mkdirSync(join(dir, 'accessibility'), { recursive: true });
        const destination = join(dir, 'accessibility', name);
        try {
          if (image.body) writeFileSync(destination, image.body);
          else cpSync(image.path, destination);
          evidence.screenshot = 'accessibility/' + name;
        } catch { evidence.captureError = 'Screenshot attachment could not be copied.'; }
      }
      return [{ ...evidence, attempt: index + 1 }];
    } catch { return [{ attempt: index + 1, captureError: 'Accessibility evidence attachment could not be read.' }]; }
  });
}
