...
<details><summary>Known routes not selected (${coverage.state === 'complete' ? coverage.notSelected.length : 'unknown'})</summary><p>Use --coverage=exhaustive to include the eligible published content inventory.</p>${json(coverage.notSelected)}</details>
...
${r.differences.some(d => d.comparison === 'unavailable') ? `<details open><summary>Structural comparison unavailable</summary><p>The captured content regions are not comparable. This is a comparison limitation, not evidence that the candidate added or removed all the listed structures.</p>${json(r.differences.filter(d => d.comparison === 'unavailable'))}</details>` : ''}
