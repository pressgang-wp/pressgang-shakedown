...
function imageSummary(reference = [], candidate = []) {
  const rows = [], count = Math.max(reference.length, candidate.length);
  for (let i = 0; i < count; i++) {
    ...
    else if (!sameSource) rows.push(`${name}: same rendered size (${after.width} × ${after.height}); source changed.`);
  }
