...
function renderDifferences(differences, json) {
  ...
  return `<ul class="finding-list">${differences.map(d => `<li><strong>${escape(label(d.key))}:</strong> ${describeDifference(d).map(escape).join(' ') }${d.suppressed ? ' <em>Accepted difference.</em>' : ''}${acceptanceHelp(d)}<details><summary>Technical evidence</summary>${json(d)}</details></li>`).join('')}</ul>`;
}
