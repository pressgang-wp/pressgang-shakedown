...
async function capture(browser, route, side, viewport, plan, dir, id) {
  ...
  try {
    ...
    if (side === 'candidate' && route.expect === 200) {
      ...
      result.accessibility = await captureAccessibilityEvidence(page, a11y, join(dir, `${id}-accessibility.png`), { timeout: plan.options.timeout, mask: plan.options.ignoreSelectors.map(s => page.locator(s)) });
