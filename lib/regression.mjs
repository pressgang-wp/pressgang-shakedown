...
import { inventoryRoutes, addCoverage } from './coverage.mjs';
...
import { pairedPlan, routePath, classifyRoute, compareEvidence, focusRoutes } from './regression-plan.mjs';
...
import { captureAccessibilityEvidence, captureElementEvidence } from './accessibility-evidence.mjs';
...
async function capture(browser, route, side, viewport, plan, dir, id) {
  ...
  try {
    ...
    if (scope.screenshots) {
      ...
      if (!result.elements.screenshot) await page.screenshot({ path: join(dir, screenshot), fullPage: true, animations: 'disabled', timeout: plan.options.timeout, mask: plan.options.ignoreSelectors.map(s => page.locator(s)) });
...
export async function runRegression(target, workspace) {
  ...
  try {
    ...
    writeFileSync(join(dir, 'plan.json'), JSON.stringify(run.plan, null, 2));
    focusRoutes(run.plan, run.coverage);
